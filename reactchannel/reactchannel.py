...
class ReactChannel(commands.Cog):
    ...
    default_react_channel_settings: ClassVar[dict] = {
        # Can be "vote", "checklist", or a list of ("emoji", "action") tuples.
    ...
    def __init__(self, bot: Red) -> None:
        ...
        self.config.register_custom(
            "REACT_CHANNEL",
            **self.default_react_channel_settings,
        )
    ...
    async def checklist(
        ...
        channel: (discord.TextChannel | discord.Thread | discord.ForumChannel | None) = None,
    ) -> None:
    ...
    async def vote(
        ...
        """All user messages will have an up and down arrow. Clicking affects a user's karma."""
    ...
    async def custom(
        ...
        *,
        ...
        """
        await self._save_channel(ctx, channel, list(dict.fromkeys(emojis.split())))
    ...
    async def _save_channel(
        ...
        if isinstance(reaction_template, list):
            ...
            try:
                ...
                await ctx.send(
                    error(
                        "That emoji is not a valid emoji I can use!"
                        if len(reaction_template) == 1
                        else "One of those emojis is not a valid emoji I can use!",
                    ),
                )
                return
            await self.config.custom(
    ...
    async def users(
        ...
        if not reaction_template:
            ...
            await ctx.send(
                success(
                    f"{channel.mention} ReactChannel will "
                    f"{'now' if react_to_users else 'no longer'} "
                    "automatically react to users.",
                ),
            )
    ...
    async def bots(
        ...
        if not reaction_template:
            ...
            await ctx.send(
                success(
                    f"{channel.mention} ReactChannel will "
                    f"{'now' if react_to_bots else 'no longer'} "
                    "automatically react to bots.",
                ),
            )
    ...
    async def myself(
        ...
        if not reaction_template:
            ...
            await ctx.send(
                success(
                    f"{channel.mention} ReactChannel will "
                    f"{'now' if react_to_myself else 'no longer'} "
                    f"automatically react to my ({channel.guild.me.display_name}) messages.",
                ),
            )
    ...
    async def role_add(
        ...
        if not reaction_template:
            ...
            await ctx.send(
                success(
                    f"{channel.mention} ReactChannel will "
                    f"{'only' if react_roles_allow else 'not'} "
                    f"react to users with the given roles:\n\n"
        ...
        if not reaction_template:
            ...
            if react_role_ids:
                ...
                await ctx.send(
                    warning(
                        f"{channel.mention} ReactChannel will "
                        f"{'only' if react_roles_allow else 'not'} react to users "
                        "with specific roles. "
                        "You don't have any roles set up though, so this will not take effect. "
                        "Use `[p]reactchannelset filter role add` to begin "
    ...
    async def text(
        ...
        if not reaction_template:
            ...
            await ctx.send(
                success(
                    f"{channel.mention} ReactChannel will "
                    f"{'now' if react_filter_text else 'no longer'} "
                    "automatically react to text-only messages.",
                ),
            )
    ...
    async def content_commands(
        ...
        if not reaction_template:
            ...
            await ctx.send(
                success(
                    f"{channel.mention} ReactChannel will "
                    f"{'now' if react_filter_commands else 'no longer'} "
                    "automatically react to command messages.",
                ),
            )
    ...
    async def images(
        ...
        if not reaction_template:
            ...
            await ctx.send(
                success(
                    f"{channel.mention} ReactChannel will "
                    f"{'now' if react_filter_images else 'no longer'} "
                    "automatically react to images.",
                ),
            )
    ...
    async def karmatop(self, ctx: commands.Context) -> None:
        ...
        for data in all_guild_members_sorted_list:
            ...
            if member:
                added += 1
                message += (
                    f"{str(added).rjust(3)}  | {member.display_name[:32].ljust(32)} | "
                    f"{data[1]['karma']}\n"
                )
    ...
    async def upvote(self, ctx: commands.Context) -> None:
        ...
        if upvote:
            await ctx.send(
                f"This servers upvote emoji is {upvote}. "
                "React to other members messages to give them karma!",
            )
        else:
    ...
    async def downvote(self, ctx: commands.Context) -> None:
        ...
        if downvote:
            await ctx.send(
                f"This servers downvote emoji is {downvote}. "
                "React to other members messages to remove karma.",
            )
        else:
    ...
    async def on_message(self, message: discord.Message) -> None:
        ...
        if not message.channel.permissions_for(message.guild.me).add_reactions:
            return
        # Get reaction configuration for channel, thread, or forum
        ...
        if not react_config["reaction_template"]:
            return
        # react_to check
        if message.author == message.guild.me:
            if not react_config["react_to"]["myself"]:
                return
        elif message.author.bot:
            if not react_config["react_to"]["bots"]:
                return
        elif not react_config["react_to"]["users"]:
            return
        # react_roles check
        react_role_ids = react_config["react_roles"]
        if react_role_ids:
            if not isinstance(message.author, discord.Member):
                return
            member_role_ids = [role.id for role in message.author.roles]
            has_matching_role = any(role_id in react_role_ids for role_id in member_role_ids)
            react_roles_allow = react_config["react_roles_allow"]
            # If the user has a matching role, and we are denying roles, or if they don't,
            # and we are allowing roles.
        ...
        if ctx and ctx.valid:
            # command
            if not react_config["react_filter"]["commands"]:
                return
        elif (
            message.attachments
            ...
            # image
            if not react_config["react_filter"]["images"]:
                return
        elif not react_config["react_filter"]["text"]:
            # text
            return
        # Actually do reactions now!
        if react_config["reaction_template"] == "checklist":
            # checklist
            await message.add_reaction("\N{WHITE HEAVY CHECK MARK}")
        elif react_config["reaction_template"] == "vote" and not message.author.bot:
            # vote
        ...
        elif isinstance(react_config["reaction_template"], list):
            # Custom reactions
            for emoji_tuple in react_config["reaction_template"]:
                with suppress(discord.HTTPException):
    ...
    async def on_raw_reaction_add(
        ...
        if not member or not payload.message_id:
            return
        # Ignore bots
        if member.bot:
            return
        ...
        if (
            ...
            react_config = await self.config.custom(
                "REACT_CHANNEL",
                str(guild.id),
                str(channel.parent.id),
            ).all()  # Returns default values
        if not react_config["reaction_template"]:
            return
        # Get reaction_template
        reaction_template = react_config["reaction_template"]
        # Get message
        try:
        ...
        if not message:
            return
        # Process checklist
        if str(payload.emoji) == "\N{WHITE HEAVY CHECK MARK}" and reaction_template == "checklist":
        ...
        if karma:
            ...
            if (
                ...
                # Bots can't get karma, only members of the guild, and members can't
                # upvote themselves.
                return
    ...
    async def on_guild_channel_delete(
        ...
        """Clean up config when a ReactChannel (channel) is deleted."""
        await self.config.custom(
            "REACT_CHANNEL",
            str(guild_channel.guild.id),
            str(guild_channel.id),
        ).clear()

    @commands.Cog.listener()
    async def on_raw_thread_delete(self, event: discord.RawThreadDeleteEvent) -> None:
        """Clean up config when a ReactChannel (thread) is deleted."""
        await self.config.custom(
            "REACT_CHANNEL",
            str(event.guild_id),
            str(event.thread_id),
