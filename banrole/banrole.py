...
class BanRole(commands.Cog):
    ...
    default_role: ClassVar[dict[str, list]] = {"banned_members": []}
    ...
    async def red_delete_data_for_user(self, *, requester: RequesterTypes, user_id: int) -> None:
        if requester == "discord_deleted_user" or requester == "owner":
            ...
            log.info(
                "Requester type is user, user_strict, or unknown: "
                "not removing user id due to operational needs",
            )
            return
        ...
        async with self.config.role(role).banned_members() as banned_list:
            for member in role.members:
                try:
                    ...
                    if mod_cog:
                        mod_conf = mod_cog.config.guild(ctx.guild)
                        respect_hierarchy = await mod_conf.respect_hierarchy()
                    else:
                        respect_hierarchy = True
                    if respect_hierarchy:
                        assert (
                            ctx.author.top_role > member.top_role or ctx.author == ctx.guild.owner
                        )
                    await ctx.guild.ban(member)
                except (discord.HTTPException, AssertionError):
                    failure_list.append(f"{member} (id {member.id})")
                else:
