...
class Verify(commands.Cog):
    ...
    __author__: ClassVar[list[str]] = ["SharkyTheKing", "Predeactor"]
    __version__ = "1.0.2"
    ...
    async def verifyset(self, ctx: commands.Context):
        ...
        if ctx.invoked_subcommand is None:
            ...
            autoroles = f"See `{ctx.prefix}verifyset autorole list` for a list of roles given."
            if role_config[0]:
                temprole = discord.utils.get(ctx.guild.roles, id=role_config[0])

            if logs is None:
                log_info = (
                    f"No channel for logging has been set, use `{ctx.prefix}verifyset log`first."
                )
            else:
    ...
    @verifyset.command()
    async def active(self, ctx: commands.Context, toggle: bool | None = None):
        """
        ...
        if not role_config[1]:
            role_config[1] = None
        if toggle is None:
            message = f"The Verification settings is set to {await tog.toggle()}."
        ...
        await ctx.send(f"Verification settings is now set to {toggle}.")
        return None
    ...
    async def temprole(self, ctx: commands.Context, *, role: discord.Role = None):
        ...
        if role:
            ...
            await ctx.send(
                f"Set the role to {role.mention}.",
    ...
    async def add_roles(self, ctx: commands.Context, *roles: discord.Role):
        ...
        for role in roles:
            if role >= ctx.author.top_role:
                errored += f"{role.name}: You can't set a role equal to or higher than your own.\n"
                continue
            if role >= ctx.guild.me.top_role:
                errored += (
                    f"{role.name}: You can't set a role that's equal to or higher than the bot.\n"
                )
                continue
        ...
        message += errored
        if added:
            message += f"\nAdded role(s): {humanize_list(added)}"
        if already_added:
            message += f"\nRole(s) already added: {humanize_list(already_added)}"
        if message:
    ...
    async def remove_roles(self, ctx: commands.Context, *roles: discord.Role):
        ...
        if not_found:
            message += f"\nRole(s) not found in autorole list: {humanize_list(not_found)}"
        if removed:
            message += f"\nRole(s) remove from autorole list: {humanize_list(removed)}"
        if message:
            for line in pagify(message):
                await ctx.send(line)
        return None
    ...
    async def list_roles(self, ctx: commands.Context):
        ...
        for role in all_roles:
            ...
            if not fetched_role:
                ...
                continue
            message += f"- {fetched_role.name} (`{fetched_role.id}`).\n"
        if maybe_not_found:
    ...
    async def verify_agree(self, ctx: commands.Context):
        ...
        try:
            ...
            try:
                if getattr(ctx.message, "delete", None) is not None:
            ...
            return self.log.warning(f"HTTPException: {e.status} - {e.code}")
        if log_config is not None:
            ...
            try:
                ...
                return self.log.warning(
                    f"Error: Unable to send log message to {ctx.bot.get_channel(log_config)}",
                )
            except discord.HTTPException as e:
                return self.log.warning(f"HTTPException: {e.status} - {e.code}")
        return None
